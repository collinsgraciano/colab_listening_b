...
### Project
- [2026-08-12 13:43:09] [project] colab_listening_b: Colab 上运行的英语听力练习视频生成管线（方案 B）。流程：LLM 脚本(SenseNova)→frontier 图片+Kokoro/edge-tts TTS→方案 B 分组(连续行合并为一个视频片段)→Seedance2 视频→FFmpeg+Pillow 合成。18 行对话，繁体中文，720p 16:9。**Why:** 用户要 Colab 版本脱离 Windows 依赖。**How to apply:** pipeline.py 是主入口，--mcp-token 传入 OAuth token，--api-key 传入 SenseNova API key，--structure original|enhanced 选择结构；grouping_b.py 按音频时长合并连续行；video_compose.py 的 compose_listening 用 group_info/line_to_group 跳过已处理的行。GitHub 公开仓库：https://github.com/collinsgraciano/colab_listening_b.git。Notebook Cell 2 git clone 自动下载代码。clip_0 generate_audio=True，其余 clip generate_audio=False。两种结构：(1) original 4章节（标题→对话→跟讀1EN×3→ZH→1EN→sil→outro）；(2) enhanced 7章节（词汇预习6词→标题→对话→慢速重听atempo=0.75→理解检查3题→跟讀1EN×2→ZH→1EN→sil→outro），代码在 enhanced/ 目录，基于 Underwood 三阶段听力教学法。
...
- [2026-08-14 15:04:55] - colab_listening_b 多Token轮换+断点续传机制：(1) mcp_client.py 支持多token，API返回"积分"/"credit"/"余额"/"quota"时自动切换下一个token，全部耗尽抛ALL_MCP_TOKENS_EXHAUSTED。(2) pipeline.py 每步完成后保存checkpoint.json，--resume标志跳过已完成步骤。步骤: step0_script→step2_images_tts→step3_video→step4_timeline→step4.5_thumbnail→step5_compose。(3) 完成后自动删除checkpoint.json，下次--resume扫描子目录找未完成的，找不到则选新topic。(4) 每个视频产物存独立子文件夹(以YouTube title命名)，包括images/clips/audio/subtitles/videos/script.json/thumbnail.jpg。(5) 视频文件也以YouTube title命名(非final_video.mp4)。(6) Notebook单Cell，MCP_TOKENS为数组格式，输出到Google Drive。--mcp-tokens替代--mcp-token(逗号分隔)。
