...
### Project
- [2026-08-16 01:12:36] [project] colab_listening_b: Colab 上运行的英语听力练习视频生成管线（方案 B）。流程：LLM 脚本(SenseNova)→frontier 图片+Kokoro/edge-tts TTS→方案 B 分组(连续行合并为一个视频片段)→Seedance2 视频→FFmpeg+Pillow 合成。18 行对话，繁体中文，720p 16:9。pipeline.py 已重构为步骤函数架构：main() 仅 ~75 行编排，_step0_script/_step1_mcp/_step2_images_tts/_step3_clips/_step4_timeline/_step45_thumbnail/_step5_compose/_step6_4k 独立可测；纯函数 _build_scene_clip_task/_build_group_clip_tasks/_enrich_timeline/_safe_dirname（新增步骤写新 _stepN_* 并在 main() 加一行调用）。**Why:** 用户要 Colab 版本脱离 Windows 依赖；巨型 main() 无法单测。**How to apply:** --mcp-tokens 传 token，--api-key 传 SenseNova key，--structure original|enhanced|static，--model deepseek-v4-flash|glm-5.2。GitHub 公开仓库：https://github.com/collinsgraciano/colab_listening_b.git。三种结构：(1) original 4章节；(2) enhanced 7章节（词汇+慢速+测验，enhanced/ 目录）；(3) static 全图片模式（compose_static，dialogue_img_{i}.png 逐行图片，跳过视频生成）。
