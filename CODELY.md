...
- [2026-08-20 15:04:12] MCP poll_task 解析 atlas（image-to-image with image_urls）图片 URL 时，返回的 JSON 字段可能与普通文生图不同。**Why:** atlas 任务用了 image_urls 参数做 image-to-image，completed 响应中 URL 可能出现在 result.url / data.url / data.imageUrl / data.image_urls 等字段，而非标准的 result.video_url / result.image_url / data.imageUrls。**How to apply:** mcp_client.py poll_task() 现在检查所有可能的 URL 字段（result_obj.url, data.url, data.imageUrl, data.image_urls），不再只查 video_url/image_url/imageUrls。同时 data.message 不再被误判为 error（只在 status=failed 时才当 error）。
...
- [2026-08-20 11:59:36] colab_listening_b YouTube title format: 繁中+emoji, 【】bracket tag, ｜separator, 80-150 chars. Three patterns: (A) 【沉浸式英文動畫】hook+emoji+topic：skills，聽完就能說！｜English topic; (B) 【每天50句英文】emoji+topic情境對話｜🎬沉浸式英文動畫｜...; (C) 【🎬沉浸式英文動畫】emoji+topic英文｜✅skill｜🗣️small talk｜... End with ｜{English topic name}. **Why:** videos target overseas Chinese audience, need high-CTR 繁中 titles. **How to apply:** LLM prompt in llm_client.py and quest/llm_client_quest.py specifies these patterns.（注：enhanced/ 目录已于 2026-08-20 删除）
