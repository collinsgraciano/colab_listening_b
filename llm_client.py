...
- Every dialogue line MUST include:
  ...
  - "poses": an array of exactly 2 pose descriptions for stop-motion animation. Each entry is a short prompt fragment describing the character in a different visual state. MUST include the character's physical description (same as char_a/char_b_description). MUST describe a DIFFERENT body language or gesture for each pose. The first pose should show the character actively speaking (mouth open, gesturing). The second pose should show the character in a listening or reacting state (e.g. head tilted, nodding, slight smile). Example: ["a young woman with brown hair in a green apron, speaking with mouth open, gesturing with right hand, coffee shop counter", "a young woman with brown hair in a green apron, listening with a slight smile, hands resting on counter, coffee shop counter"]. CRITICAL: the character's physical appearance (hair, clothing, accessories) MUST be IDENTICAL in both poses — only the gesture/expression changes.
